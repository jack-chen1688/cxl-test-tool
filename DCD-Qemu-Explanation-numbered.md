...
- [How DCD works in Qemu Emulation - One VM case](#how-dcd-works-in-qemu-emulation---one-vm-case)
  - [Create region](#create-region)
    - [1.1.1 Check memdev size](#111-check-memdev-size)
    - [1.1.2 create a region based on the sizeg](#112-create-a-region-based-on-the-sizeg)
    - [1.1.3 Kernel Log](#113-kernel-log)
  - [Add Dynamic Capacity](#add-dynamic-capacity)
    - [1.2.1 QMP command sent by the cxl-tool.py](#121-qmp-command-sent-by-the-cxl-toolpy)
    - [1.2.2 QEMU Handles the cxl-add-dynamic-capacity QMP command](#122-qemu-handles-the-cxl-add-dynamic-capacity-qmp-command)
    - [1.2.3 Kernel processes the DCD event](#123-kernel-processes-the-dcd-event)
    - [1.2.4 Qemu add the extent accepted to the device](#124-qemu-add-the-extent-accepted-to-the-device)
  - [Show Extents](#show-extents)
    - [1.3.1 QMP commands issued by the cxl-tool.py](#131-qmp-commands-issued-by-the-cxl-toolpy)
    - [1.3.2 Qemu returns extents](#132-qemu-returns-extents)
  - [Release Extent](#release-extent)
    - [1.4.1 QMP commands issued by the cxl-tool.py](#141-qmp-commands-issued-by-the-cxl-toolpy)
    - [1.4.2 Qemu handles cxl-release-dynamic-capacity QMP command](#142-qemu-handles-cxl-release-dynamic-capacity-qmp-command)
    - [1.4.3 Kernel processes the DCD event](#143-kernel-processes-the-dcd-event)
    - [1.4.4 Qemu mem device releases the Extent](#144-qemu-mem-device-releases-the-extent)
...
```
### 1.1.2 create a region based on the sizeg 
...
### 1.2.1 QMP command sent by the cxl-tool.py

Below is the output of the command to add an extent of 0-128MB
```
...
### 1.2.2 QEMU Handles the cxl-add-dynamic-capacity QMP command
...
### 1.2.3 Kernel processes the DCD event
...
### 1.2.4 Qemu add the extent accepted to the device
...
### 1.3.1 QMP commands issued by the cxl-tool.py
...
### 1.4.1 QMP commands issued by the cxl-tool.py
...
```
### 1.4.2 Qemu handles cxl-release-dynamic-capacity QMP command
...
### 1.4.3 Kernel processes the DCD event
...
### 1.4.4 Qemu mem device releases the Extent
